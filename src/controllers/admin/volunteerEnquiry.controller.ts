...
export class AdminVolunteerEnquiryController {
    ...
    async getAllEnquiries(@Req() req: Request, @Res() response: Response) {
        try {
            ...
            const result = await this.volunteerEnquiryRepository.aggregate(pipeline).toArray();
    ...
    async getEnquiryById(@Param("id") id: string, @Res() response: Response) {
        try {
            const enquiry = await this.volunteerEnquiryRepository.findOneBy({
                _id: new ObjectId(id),
                isDelete: 0
            });

            if (!enquiry) {
                return response.status(404).json({ message: "Volunteer enquiry not found" });
            }

            return response.status(200).json({ data: enquiry, message: "Volunteer enquiry fetched successfully" });
        } catch (error) {
            return handleErrorResponse(error, response);
        }
    }
