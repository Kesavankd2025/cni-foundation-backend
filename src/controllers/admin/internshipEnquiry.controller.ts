...
export class AdminInternshipEnquiryController {
    ...
    async getAllEnquiries(@Req() req: Request, @Res() response: Response) {
        try {
            ...
            const result = await this.internshipEnquiryRepository.aggregate(pipeline).toArray();
    ...
    async getEnquiryById(@Param("id") id: string, @Res() response: Response) {
        try {
            const enquiry = await this.internshipEnquiryRepository.findOneBy({
                _id: new ObjectId(id),
                isDelete: 0
            });

            if (!enquiry) {
                return response.status(404).json({ message: "Internship enquiry not found" });
            }

            return response.status(200).json({ data: enquiry, message: "Internship enquiry fetched successfully" });
        } catch (error) {
            return handleErrorResponse(error, response);
        }
    }
